...
// entrevistada ao vivo para um jornal da cidade. No entanto, ficou um pouco nervosa na hora, e diante da
// situação, sua fala ficou um pouco distorcida, repetindo o final de cada palavra após dizer a mesma.
...
public class EntrevistaEmbaracosa {
    public static void main(String[] args) {
        ...
        while (scanner.hasNext()) {
            ...
            int limit = (int) Math.floor(word.length() / 2);
            int end = word.length();
            int aux = 0;
            ...
            if (word.length() == 1) {
                ...
                for (int i = limit; i >= 1; i--) {
                    if (word.substring(end - i, end).equals(word.substring(end - (2 * i), end - i))) {
                        aux = i;
                        System.out.println(word.substring(0, (end - aux)));
                    } else if (limit == 1) {
                        System.out.println(word);
                    }
                }

                if (limit > 1 && aux == 0) System.out.println(word);
            }
        }
        scanner.close();
    }
